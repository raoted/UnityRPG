...
public class RnMUI_IconSlotInspector : Editor
{
	...
	public void DrawBehaviour()
	{
		...
		if (NGUIEditorTools.DrawHeader("Behaviour"))
		{
			...
			RnMUI_IconSlot.HoverEffectType hoverEffectType = (RnMUI_IconSlot.HoverEffectType)this.serializedObject.FindProperty("hoverEffectType").enumValueIndex;
			
			if (hoverEffectType == RnMUI_IconSlot.HoverEffectType.Sprite)
			{
				EditorGUILayout.PropertyField(this.serializedObject.FindProperty("hoverEffectSprite"), new GUIContent("Hover Sprite"));
			}
			else if (hoverEffectType == RnMUI_IconSlot.HoverEffectType.Color)
			{
			...
			if (hoverEffectType != RnMUI_IconSlot.HoverEffectType.None)
			{
			...
			RnMUI_IconSlot.PressEffectType pressEffectType = (RnMUI_IconSlot.PressEffectType)this.serializedObject.FindProperty("pressEffectType").enumValueIndex;EditorGUILayout.PropertyField(this.serializedObject.FindProperty("hoverEffectType"), new GUIContent("Hover Effect Type"));

			if (pressEffectType == RnMUI_IconSlot.PressEffectType.Sprite)
			{
				EditorGUILayout.PropertyField(this.serializedObject.FindProperty("pressEffectSprite"), new GUIContent("Press Sprite"));
			}
			else if (pressEffectType == RnMUI_IconSlot.PressEffectType.Color)
			{
			...
			if (pressEffectType != RnMUI_IconSlot.PressEffectType.None)
			{
